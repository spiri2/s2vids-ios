...
//  - Discord update falls back to email if session user id is missing.
...
struct SettingsView: View {
  ...
  @State private var trialEnd: Int = 0       // ← store trial_end so we can check access
  ...
  // Has a valid subscription or a trial
  ...
  var body: some View {
      ...
      ScrollView {
        VStack(spacing: 16) {
          ...
          VStack(spacing: 16) {
            ...
            invitePanelView()
            accountPanelView()
            systemPanelView() // App Version + Changelog
          }
          ...
          if !confirmation.isEmpty {
            Text(confirmation)
              ...
              .foregroundColor(confirmationKind == .error
                               ? Color(red: 1.0, green: 0.65, blue: 0.65)
                               : Color(red: 0.67, green: 0.93, blue: 0.80))
    ...
    // 👇 Swipe-from-left-edge to dismiss (consistent with Movies/Admin/Discover)
    .highPriorityGesture(
  ...
  private var header: some View {
    HStack(spacing: 12) {
      ...
      // 👇 Dropdown menu; dismiss FIRST to avoid stacked screens
      UserMenuButton(
        email: email,
        isAdmin: isAdmin,
        onRequireAccess: { /* no-op here (settings) */ },
        onLogout: {
        ...
        onOpenMovies: {
          dismiss()
          NotificationCenter.default.post(name: .init("S2VidsOpenMovies"), object: nil)
        },
        onOpenDiscover: {
          dismiss()
          NotificationCenter.default.post(name: .init("S2VidsOpenDiscover"), object: nil)
        },
        onOpenTvShows: {
          dismiss()
          NotificationCenter.default.post(name: .init("S2VidsOpenTvShows"), object: nil)
        },
        onOpenAdmin: {
          dismiss()
          NotificationCenter.default.post(name: .init("S2VidsOpenAdmin"), object: nil)
        }
      )
    }
    .zIndex(10_000)
  ...
  // Styled external link that looks like our primary button
  private func primaryLink(_ title: String, url: String) -> some View {
  ...
  // MARK: Jellyfin / Stripe (2×2 buttons: top row = Launch/Request, bottom = Change-Password(or Create)/Cancel)
  ...
  private func jellyfinPanelView() -> some View {
    sectionContainer {
      ...
      // 2×2 grid of buttons
      VStack(spacing: 10) {
        // TOP ROW: Launch + Request (only when user has access)
        HStack(spacing: 10) {
          if hasAccess {
            primaryLink("Launch Jellyfin", url: "https://atlas.s2vids.org/")
          }
          if hasAccess {
            primaryLink("Request Media", url: "https://req.s2vids.org/")
          }
        }

        // BOTTOM ROW: Change/Create + Cancel
        HStack(spacing: 10) {
          ...
          } else if !jfExists() && canGenerateJF() {
  ...
  // MARK: System (App Version + Changelog)

  private func systemPanelView() -> some View {
    sectionContainer {
      Text("System").font(.system(size: 17, weight: .bold)).foregroundColor(.white)

      labeledRow("App Version") {
        Text(appVersionDisplay).foregroundColor(.white)
      }

      labeledRow("Changelog") {
        Link("View",
             destination: AppConfig.apiBase.appendingPathComponent("changelog"))
  ...
  private func updateDiscord() async {
    ...
    struct ById: Encodable { let id: String; let discord: String }
    struct ByEmail: Encodable { let email: String; let discord: String }

    do {
      let (data, resp): (Data, HTTPURLResponse)
      if !userId.isEmpty {
        (data, resp) = try await postJSON(apiURL("api/save-user"), body: ById(id: userId, discord: trimmed))
      } else {
        (data, resp) = try await postJSON(apiURL("api/save-user"), body: ByEmail(email: email, discord: trimmed))
      }
